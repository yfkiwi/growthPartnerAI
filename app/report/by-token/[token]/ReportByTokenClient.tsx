      ...
      <div className="py-12">
        <div className="max-w-4xl mx-auto px-4">
          <div className="text-center mb-12">
            <h1 className="text-3xl md:text-4xl font-extrabold mb-4">{getReportTypeTitle(submission.report_type)}</h1>
            <p className="text-lg text-gray-300 mb-6">Analysis for: &quot;{submission.idea}&quot;</p>
            <div className="flex items-center justify-center space-x-4 text-sm text-gray-400">
              <span>📧 {submission.email}</span>
              <span>📅 {new Date(submission.created_at).toLocaleDateString('en-CA')}</span>
              <span className={`px-2 py-1 rounded text-xs ${submission.status === 'completed' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>{submission.status}</span>
            </div>
          </div>
            ...
            {isPending ? (
              ...
              <div className="text-center">
                <div className="bg-gray-700 p-8 rounded-lg mb-8">
                  ...
                  <p className="text-gray-300 mb-6">Unlock the complete analysis with detailed insights, market research, competitive analysis, and actionable recommendations.</p>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <button onClick={() => handlePurchase('single')} className="bg-blue-600 text-white font-bold py-4 px-8 rounded-lg hover:bg-blue-700 transition-colors">Unlock for $29</button>
                    <button onClick={() => handlePurchase('bundle')} className="bg-green-600 text-white font-bold py-4 px-8 rounded-lg hover:bg-green-700 transition-colors">Get All 5 Reports - $99</button>
                  </div>
                </div>
